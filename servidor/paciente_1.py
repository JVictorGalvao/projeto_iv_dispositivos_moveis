...
def atualiza_batimentos(batimento: int):
    ...
    r = requests.put('http://127.0.0.1:3000/paciente/1/batimentos', data=json.dumps(payload), headers=headers)
...
def atualiza_oxigenacao(oxigenacao: int):
    ...
    r = requests.put('http://127.0.0.1:3000/paciente/1/oxigenacao', data=json.dumps(payload), headers=headers)
...
def atualiza_ultrassom(ultrassom: str):
    ...
    r = requests.put('http://127.0.0.1:3000/paciente/1/ultrassom', data=json.dumps(payload), headers=headers)
...
def atualiza_dados(batimento: int, oxigenacao: int):
    ...
    r = requests.put('http://127.0.0.1:3000/paciente/1/dados', data=json.dumps(payload), headers=headers)
